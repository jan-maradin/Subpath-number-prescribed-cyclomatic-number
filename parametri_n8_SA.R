...
graf <- ggplot(graf_1, aes(x = `µ(G)`, y = vrednosti, color = parametri)) +
  geom_line() +
  geom_point()
graf
...
# predpostavka: graf_1 ima stolpce: n, `µ(G)`, parametri, vrednosti
# in ena od vrednosti parametri je "min p_n(G)_pravilno"
...
ggplot(graf_2, aes(x = `µ(G)`, y = odmik, color = parametri, group = parametri)) +
...
# graf_1: n, µ(G), parametri, vrednosti
# ena izmed vrednosti v `parametri` je "min p_n(G)_pravilno"
