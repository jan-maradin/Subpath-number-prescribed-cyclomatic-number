...
\section*{1.~del projektne naloge}
    V prvem delu projektne naloge sva želela preveriti, kateri način simuliranega ohlajanja (SA) je najučinkovitejši, zato sva natančno iskanje grafov
    z generacijo vseh izomorfnih grafov in algoritem simuliranega ohlajanja testirala do iste velikosti grafov. 
    Za dano število vozlišč in za vsa pripadajoča ciklomatska števila sva poiskala samo grafe z najmanjšim oziroma največjim številom podpoti, 
    torej se nisva osredotočala na geodetske podpoti.
    Ker sva grafe iskala za vsa možna ciklomatska števila, je bil postopek iskanja zelo časovno zahteven, 
    zato sva se omejila na grafe z največ 9 vozlišči.
...
\subsection*{Iskanje točnih vrednosti}
\begin{itemize}
    \item Koda za ta del je zapisana v datoteki \href{run:../1del_mali_grafi.ipynb}{\texttt{1del\_mali\_grafi.ipynb}}
    \texttt{1del\_mali\_grafi.ipynb}, rezultati pa so v \texttt{1del\_mali.csv}
...
\subsection*{1.del z SA}
    Zanimalo naju je kako dobro deluje SA, če spremeniva določene parametre. 
    Rezultate pa sva primerjala z točnimi vrednostmi, ki sva jih izračunala v prejšnjem delu.


\fbox{$n = 9$}
\begin{itemize}
    \item Najprej sva se osredotočila na grafe z n = 9 in vemi možnimi mu-ji z nastavitvami \texttt{max\_steps = 300}, \texttt{T\_start = 3}, 
    \texttt{T\_end = 0.001}, \texttt{cooling = 0.99}. Torej sva vbistvu iskala kateri dober priblićek bi dala v SA kot začetno vrednost, da bi bila
     čim večja verjetnost da se algoritem nebi ustavil v lokalnem ekstremu.
    \item Koda za ta del je zapisana v datoteki \texttt{1del\_SA\_n\_9.ipynb}, rezultati pa v \texttt{1del\_SA\_n\_9.csv},
    \item Koda se je izvajala približno 35 min
    \item Začetne približke sva skonstruirala iz točnih vrednosti prejšnega dela, tako da sva vzela graf z 8 vozlič in istim ciklomatskim številom, 
    ter mu dodala vozlišče in eno povezavo tako, da je graf ostal povezan. Ko pa sva pognala SA na ciklomatskem številu, ki je večji od 21 
    ...
    povezavo tako da se ciklomatsko število za 1 poveča.
